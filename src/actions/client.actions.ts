...
export async function deleteClient(userId: string): Promise<DeleteActionResult> {
  if (!userId) {

    redirect(`/dashboard/clients?status=error&message=${encodeURIComponent('Missing Client ID for deletion.')}`);
  }
  ...
  try {
    ...
    redirect(`/dashboard/clients?status=success&name=${encodeURIComponent(userName)}&action=deleted`);
    ...
    redirect(`/dashboard/clients?status=error&message=${encodeURIComponent('Failed to delete user. Check for active sessions or related records.')}`);
  }
}