  ...
  if (!task || task.project.id !== projectId) {
    return (
      <div className="p-8 text-red-500">
        ...
        <Link href={`/dashboard/projects/${projectId}`} className="text-primaryRed hover:underline mt-4 block">
          ← Back to Tasks
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-8 bg-zBlack min-h-screen text-white">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Edit Task – <span className="text-primaryRed">{task.name}</span></h1>
        <Link href={`/dashboard/projects/${projectId}`} className="text-primaryRed hover:underline">
          ← Back
        </Link>
      </div>
